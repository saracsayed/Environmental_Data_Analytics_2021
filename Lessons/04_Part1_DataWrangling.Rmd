...
library(lubridate)
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE) #string as factors = true makes it possible to read characters in the summary
...
`>=`
`&` # Aggregate columns
`|` # Set if you want one or the other for multiple columns

```{r}
class(NTL.phys.data$lakeid) #when you're filtering you have to make sure the classes match up
class(NTL.phys.data$depth)

# matrix filtering, square brackets
...
NTL.phys.data.1999 <- filter(NTL.phys.data,sampledate == 1999)
