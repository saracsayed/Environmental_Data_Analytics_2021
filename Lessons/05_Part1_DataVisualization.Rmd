...
Always need to provide ggplot a data frame
...
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) # when you specify color within the aestehtics it adds another layer to the plot
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #facet wrap plots data points based on input (here we chose sitename)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Fill just changes the fill color in the boxplot
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) ##prevents dots from overlaying
print(Nutrientplot4)
