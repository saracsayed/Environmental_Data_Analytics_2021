...
> Answer: 
H0 = mu of all the values in our sample is greater than or equal to 50 ppm
HA = mu of the values in our sample is greater than 50 ppm
...
lm3 <- lm(data = EPAair, PM2.5~ Ozone)
summary(lm3)
...
lm_scatter <- ggplot(data= EPAair, aes(x= Ozone, y = PM2.5)) + geom_point()+
...
library(corrplot)
EPA_Corr <- cor (EPAair %>% select (SITE_LATITUDE:PM2.5) %>% na.omit())
