...
ggplot(data = BRA_complete, aes(x = Date , y = Temperature_C)) + geom_line() + geom_smooth()
...
Temp_BRA_ts <- ts(BRA_complete$Temperature_C, start = c(1901,1), frequency = 12)

Rain_BRA_ts <- ts(BRA_complete$Rainfall, start = c(1901,1), frequency = 12)
...
Temp_BRA_Decomposed <- stl(Temp_BRA_ts, s.window = "periodic")

Rain_BRA_Decomposed <- stl(Rain_BRA_ts, s.window = "periodic")

plot(Temp_BRA_Decomposed)
