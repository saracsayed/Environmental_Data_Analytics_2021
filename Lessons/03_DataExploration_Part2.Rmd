...
Visualize count data for categorical variables. 
  ...
  ggplot(USGS.flow.data, aes(x = discharge.mean.approval)) +
    geom_bar() ##Counts of records in each category
```
...
#
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10) #binwidth sets the width of the bins

#
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20) #set the number of bins
## You should generally use between 5-20 bins
#
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500)) # set a scale for the x axis, zoom in to your histogram
...
ggplot(USGS.flow.data) +
  ...
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2) + #lty = line type
